...
export const getWeatherData = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (city === "london") {
      ...
      finalWeatherData = generateBristolWeatherData();
    } else {
...
export const getThreeDayWeatherData = async (req: Request, res: Response) => {
  const errors = validationResult(req);
  ...
  try {
    ...
    // We will create a variable with a type of WeatherData
    let threeDayWeatherData: ThreeDayWeatherData;

    // We will use an if statement to check which city was passed in
    if (city === "london") {
      threeDayWeatherData = generateThreeDayWeatherData();
    } else if (city === "dublin") {
      threeDayWeatherData = generateThreeDayWeatherData();
    } else if (city === "glasgow") {
      threeDayWeatherData = generateThreeDayWeatherData();
    ...
    // We will return the weather data as JSON
    res.status(200).json(threeDayWeatherData);
  } catch (error) {
    // If there is an error, we will log it and send a 500 status code
    res.status(500).send("Error in fetching weather data");
  }
